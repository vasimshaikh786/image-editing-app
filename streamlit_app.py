...
# Set page config
st.set_page_config(
    page_title="Image Tracking Number Editor",
    page_icon=":camera:",
    layout="wide",
...
def detect_phone_numbers(image):
    ...
    for i in range(len(d['text'])):
        text = d['text'][i].strip()
        if text:
            # Check if this text is part of a phone number
    ...
    # Add the last detected number if any
    if current_number:
        phone_numbers.append({
            'number': current_number,
            'bbox': current_bbox,
...
def replace_text_in_image(image, original_text, new_text, bbox):
    ...
    font_size = int(bbox_height * 0.8)  # Initial estimate
    font = None
    
    # Try to find a font that matches the width
    for fs in range(font_size, 0, -1):
        try:
            font = ImageFont.truetype("arial.ttf", fs)
            text_width = draw.textlength(original_text, font=font)
            if text_width <= bbox_width * 1.2:  # Allow some padding
                break
        except:
    ...
    # Get the color of the original text (average color in the bbox)
    ...
    # Draw a white rectangle as background (optional)
...
def generate_proxy_number(original):
    ...
    # Simple approach: replace digits while maintaining format
...
def display_image_with_boxes(image, detected_numbers):
    ...
    if isinstance(image, np.ndarray):
        ...
        if img_display.ndim == 2:  # Grayscale
    ...
    for num in detected_numbers:
        ...
        # Put the detected number as text
